...
# FER-2013 Image Emotion Labels (7 Classes)
EMOTION_LABELS  = {0: 'Angry', 1: 'Disgust', 2: 'Fear', 
                   3: 'Happy', 4: 'Sad', 5: 'Surprise', 6: 'Neutral'}
COGLOAD_LABELS  = {0: 'Low', 1: 'Medium', 2: 'High'}

# --- NEW: EMOTION SENSITIVITY BOOSTER ---
# Keys are the class index (4 = Sad). Value is the multiplier. 
# 1.5 means we boost the model's sensitivity to "Sad" by 50%.
EMOTION_BOOST_FACTORS = {4: 1.5} 
...
class GazeHeatmap:
    ...
    def __init__(self, screen_w: int, screen_h: int,
        ...
        sigma  = radius / 2.5          
        self._kernel = np.exp(-dist2 / (2 * sigma**2))   
    ...
    def render(self, canvas: np.ndarray, gaze_x: int, gaze_y: int) -> np.ndarray:
        ...
        kernel_crop = self._kernel[k_y0:k_y1, k_x0:k_x1]   
        idx = (kernel_crop * 255).astype(np.uint8)           
        colored = self._CMAP[idx]                            
        alpha_map = (kernel_crop * self.alpha)[..., np.newaxis]   
...
def main():
    ...
    while cap.isOpened():
        ...
        # ==========================================
        # UPDATED: Model 3: Emotion (With Sensitivity Booster)
        # ==========================================
        if frame_idx % EMOTION_INTERVAL == 0:
            if face_input is not None:
                emo_tensor = tf.constant(face_input, dtype=tf.float32)
                
                # Copy the raw predictions so we can modify them
                emo_raw = np.copy(infer_emotion(emo_tensor).numpy()[0])
                
                # Apply the boost multipliers to specific emotions
                for emo_idx, multiplier in EMOTION_BOOST_FACTORS.items():
                    emo_raw[emo_idx] *= multiplier
                
                # Re-normalize so the probabilities still sum perfectly to 1.0
                emo_raw = emo_raw / np.sum(emo_raw)
