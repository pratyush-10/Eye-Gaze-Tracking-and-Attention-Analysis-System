...
# Labels
EMOTION_LABELS  = {0: 'Excited / Happy', 1: 'Calm / Relaxed',
                   2: 'Sad / Bored',     3: 'Angry / Anxious'}
...
class EmotionFeatureExtractor:
    ...
    def extract(gaze_history, head_history, lid_dist_history):
        ...
        features += [np.mean(gaze_arr[:,0]), np.std(gaze_arr[:,0]), np.var(gaze_arr[:,0]),
        ...
        features += [np.mean(gaze_vel), np.std(gaze_vel), np.max(gaze_vel),
                     np.percentile(gaze_vel, 50), np.percentile(gaze_vel, 75),
                     np.percentile(gaze_vel, 95)]

        features += [np.mean(head_arr[:,0]), np.std(head_arr[:,0]),
        ...
        fix = gaze_vel < 0.02
        fix_c = int(np.sum(fix)); sac_c = len(gaze_vel) - fix_c
        n = max(len(gaze_vel), 1)
        features += [fix_c, sac_c, fix_c/n, sac_c/n,
                     np.max(gaze_arr[:,0])-np.min(gaze_arr[:,0]),
                     np.max(gaze_arr[:,1])-np.min(gaze_arr[:,1])]

        blink_c = int(np.sum(lid_arr < 0.012))
        features += [np.mean(lid_arr), np.std(lid_arr), np.min(lid_arr), np.max(lid_arr),
                     blink_c, blink_c/max(len(lid_arr),1),
                     np.percentile(lid_arr, 50), np.percentile(lid_arr, 75)]

        if len(gaze_vel) > 1:
            ga = np.abs(np.diff(gaze_vel))
            features += [np.mean(ga), np.std(ga), np.max(ga),
                         np.percentile(ga, 50), np.percentile(ga, 90),
                         float(np.mean(np.diff(lid_arr))) if len(lid_arr) > 1 else 0.]
        else:
            features += [0.]*6

        features = features[:49]
        features += [0.]*(49 - len(features))
...
def main():
    ...
    while cap.isOpened():
        ...
        # ---- Model 3: Emotion (every EMOTION_INTERVAL frames) ----
        if frame_idx % EMOTION_INTERVAL == 0:
            emo_feat   = EmotionFeatureExtractor.extract(gaze_history, head_history, lid_history)
            emo_tensor = tf.constant(emo_feat[np.newaxis], dtype=tf.float32)
